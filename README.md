...
> **Archived research snapshot (2026-08-28).** This repository preserves the
> code, notebooks, research notes, and negative/null-result analyses from the
> original `embeding_and_GDnonPF` workspace. Large downloaded datasets,
> generated caches, Python bytecode, and local credentials are intentionally
> excluded. See `research_archive/READ_ME_FIRST.md` before citing any claim:
> later matched controls falsified the proposed incremental value of the
> Procrustes/representation metrics in the tested regime.

The early `Noised_PageRank` subproject is preserved separately at
https://github.com/cis-aconiticacid/Noised_PageRank.
